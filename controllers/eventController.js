...
const getByID = async (req, res) => {
  ...
  if (!Array.isArray(response))  // the response should be an array
    return res.status(400).json({
      success: false,
      message: `Unable to retrieve event with id ${ID}.`,
      data: response,
    });
  if (!response.length)  // it checks if the array is empty.
    return res.status(400).json({
      success: false,
...
const getEventByID = async (ID) => { //we took id as param instead of req res keeping the function  more reusable
